...
class Uploader
{
    ...
    private function upFile()
    {
        ...
        if (!(move_uploaded_file($file["tmp_name"], $this->filePath) && file_exists($this->filePath))) { //移动失败
            ...
            $object = 'ueditor/' . $this->fileName;//oss保存的文件名称
            $file = $this->filePath;//文件路径，必须是本地的。
            try{
                $ossClient->uploadFile(_OSS_BUCKET_, $object, $file); //上传文件到oss
                unlink($file); //删除本地文件
                $this->stateInfo = $this->stateMap[0];
            } catch(OssException $e) {
                //oss上传失败
                $this->stateInfo = 'OSS错误：' . $e->getMessage();
            }
        }
    }
    ...
    public function getFileInfo()
    {
        return array(
            ...
            "url" => '//' . _OSS_BUCKET_ . '.' . _OSS_ENDPOINT_ . '/ueditor/' . $this->fileName,
            "title" => $this->fileName,
